# Horizontal Bar routine child class
...
class HbarRoutine(Routine):
    # Object constructor
    def __init__(self, skills, numberOfSkills, hasEGIV, hasEGI, hasEGII, hasEGIII, isValid, countingElements, apparatus="Horizontal Bar"):
    ...
    def add_skill(self, COP_conn):
        baseQuery = "SELECT skillName, skillValue, skillNumber FROM hb_eg_"
        cursor = COP_conn.cursor()
        # Prints options for selection and reads user input
        print("I. Long hang swings with and without turns.")
        print("II. Flight elements.")
        print("III. In bar and Adler elements.")
        print("IV. Dismounts.")
        ...
        if selection != "esc":
            ...
            assignQuery = "SELECT * FROM hb_eg_" + egString + " WHERE skillNumber = '" + selection + "';"
    ...
    def make_newSkill(self, choice):
        if choice[3] != "III":
            newSkill = skill.HbarSkill(choice[0], choice[1], choice[2], choice[3], choice[4], None)
        elif choice[3] == "III":
            newSkill = skill.HbarSkill(choice[0], choice[1], choice[2], choice[3], choice[4], choice[5])
        return newSkill
